...
vector<vector<ftype>> matrix3x3Inverse(vector<vector<ftype>> &matrix) {
    vector<vector<ftype>> result{{0, 0, 0}, {0, 0, 0}, {0, 0, 0}};
...
pt3 pt3::operator-(const pt3 &o) const { return pt3(x - o.x, y - o.y, z - o.z); }
pt3 pt3::cross(const pt3 &o) const {
    return pt3(differenceOfProducts(y, o.z, z, o.y), differenceOfProducts(z, o.x, x, o.z),
               differenceOfProducts(x, o.y, y, o.x));
}
ftype pt3::dot(const pt3 &o) const { return x * o.x + y * o.y + z * o.z; }
void pt3::show() { cout << x << " " << y << " " << z << " " << endl; }
ftype pt3::dist(const pt3 &o) const { return sqrt((x - o.x) * (x - o.x) + (y - o.y) * (y - o.y)); }
...
ftype ftype_abs(ftype x) { return (x >= 0) ? x : -x; }
...
bool rightTurn(const pt3 &point1, const pt3 &point2, const pt3 &point3) {
    return differenceOfProducts((point3.x - point1.x), (point2.y - point1.y), (point3.y - point1.y),
                                (point2.x - point1.x)) > 0;
}
...
pt3 lineInter2D(pt3 A, pt3 B, pt3 C, pt3 D) {
    ...
    return pt3(differenceOfProducts(b2, c1, b1, c2) / determinant, differenceOfProducts(a1, c2, a2, c1) / determinant,
               0);
}

void orientedBoundingBox(vector<pt3> points, pt3 orientation, ftype &volume, vector<pt3> &lowerBase,
                         vector<pt3> &upperBase) {
    bool indX = ftype_abs(orientation.x) < EPS, indY = ftype_abs(orientation.y) < EPS,
         indZ = ftype_abs(orientation.z) < EPS;
    if (indX && indY && indZ) {
        return;
    }

    // Make base transformation matrix
    vector<vector<ftype>> trMatrix{{1, 0, 0}, {0, 1, 0}, {0, 0, 1}};
    ...
    if (indX && indY) {
        ...
        vector<vector<ftype>> C{{baseX.x, baseY.x, baseZ.x}, {baseX.y, baseY.y, baseZ.y}, {baseX.z, baseY.z, baseZ.z}};
        trMatrixInverse = C;
    ...
    for (int i = 0; i < nPoints; i++) {
        ...
        points[i].x = tempVector[0];
        points[i].y = tempVector[1];
        points[i].z = tempVector[2];
        if (zMin > points[i].z) {
            zMin = points[i].z;
        }
        if (zMax < points[i].z) {
    ...
    for (int i = 1; i < nPoints; i++) {
        if (ftype_abs(tempPoints[tempPoints.size() - 1].x - points[i].x) < EPS &&
            ftype_abs(tempPoints[tempPoints.size() - 1].y - points[i].y) < EPS) {
            continue;
        }
    ...
    for (int i = 2; i < nPoints; i++) {
        while (upperCH2D.size() > 1 &&
               (!rightTurn(upperCH2D[upperCH2D.size() - 2], upperCH2D[upperCH2D.size() - 1], tempPoints[i]))) {
            upperCH2D.pop_back();
        }
        upperCH2D.push_back(tempPoints[i]);
    }
    for (int i = 2; i < nPoints; i++) {
        while (lowerCH2D.size() > 1 && (!rightTurn(lowerCH2D[lowerCH2D.size() - 2], lowerCH2D[lowerCH2D.size() - 1],
                                                   tempPoints[nPoints - i - 1]))) {
            lowerCH2D.pop_back();
        }
    ...
    for (int q = 0; q < CH2D.size(); q++) {
        ...
        ftype tempArea =
            tempRectanglePoints[0].dist(tempRectanglePoints[1]) * tempRectanglePoints[1].dist(tempRectanglePoints[2]);
        if (tempArea < area) {
            area = tempArea;
            for (int i = 0; i < 4; i++) {
